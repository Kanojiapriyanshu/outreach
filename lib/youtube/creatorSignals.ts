/**
 * The signals both Discovery and the media kit read a creator through.
 *
 * These used to live privately inside channelMediaKit.ts, which meant Discovery had no way to
 * rank or filter on the same things the media kit would later tell a brand. Sharing them is the
 * point: a creator that Discovery calls "Strong brand safety" is the same computation the media
 * kit prints, so a shortlist never contradicts the document it turns into.
 *
 ...
 * title videos with. Kept as data rather than the legacy system's hardcoded per-vertical `if`
 * blocks (which only ever covered drones and pool cleaners, and silently did nothing else). */
...
/* -------------------------------------------------------------------------- */
/* Content signals (shared with the media kit)                                 */
/* -------------------------------------------------------------------------- */
 ...
 *
 * Deliberately narrow patterns: the legacy system's version rejected any channel whose text
 * mentioned "food", "comedy" or "cricket", which threw away real creators on every search.
 */
...
/**
 * A 0-100 "is this creator worth the outreach" score, weighted the same way computeBrandFitScore
 * weights the media kit's headline number — engagement and real reach over raw subscriber count,
