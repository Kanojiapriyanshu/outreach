/**
 * Numeric and formatting helpers shared by the YouTube insight pipeline.
 *
 * Ported verbatim in behaviour from the Fidem CRM's `server/utils/number.js`. The report figures
 * these produce are compared against reports generated by the old system, so the rounding and
 * bucketing rules have to match exactly rather than being "tidied up" on the way across.
 */
