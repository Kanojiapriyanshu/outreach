/**
 * The Insight OS report engine.
 *
 * Ported from the Fidem CRM's `server/services/youtubeInsight.service.js`. The analysis rules —
 * comment classification, sentiment, scoring thresholds, performance estimates — are kept
 * byte-for-byte in behaviour rather than rewritten, because reports generated here are compared
 * against reports the old system produced. A "tidier" reimplementation that scored differently
 * would be a silent regression nobody could see.
 *
 * What did change: persistence is Prisma/Postgres instead of Mongoose, and the AI verdict runs on
 * Anthropic (already wired up in this app for reply classification) rather than OpenAI.
 */
import {
  clamp,
  percent,
...
import { normalizeVideo, normalizeChannel, calculateCreatorAverage } from "./normalize";
import { buildYoutubeInsightDashboard, buildChartData } from "./reportDashboard";
import { generateYoutubeAiSummary } from "./aiSummary";
...
function includesAny(text: any, words: any) {
...
function uniqueTop(items: any, limit = 8) {
  const seen = new Set();
  const out: any[] = [];
...
function getVideoUrlFromPayload(payload: any = {}, videoId = '') {
...
function buildYoutubeOnlyInput(payload: any = {}) {
  return {
    ...
    commentOrder: (payload.commentOrder === 'time' ? 'time' : 'relevance') as 'time' | 'relevance',
    rpmLow: Math.max(0, toNumber(payload.rpmLow, 1.36)),
    rpmHigh: Math.max(0, toNumber(payload.rpmHigh, 3.4))
  };
}

function classifyComment(comment: any) {
...
function getCommentPublishedTime(comment: any = {}) {
  const date = new Date(comment.publishedAt || comment.updatedAt || 0);
  return Number.isNaN(date.getTime()) ? 0 : date.getTime();
}

function normalizeCommentForInsight(comment: any = {}, classification = null) {
...
function buildCommentTabs(comments: any[] = [], classifications: any[] = []) {
  ...
  const sortByLatest = (items: any[] = []) => [...items].sort((a, b) => getCommentPublishedTime(b) - getCommentPublishedTime(a));
  const sortBySignal = (items: any[] = []) => [...items].sort((a, b) => toNumber(b.insightScore) - toNumber(a.insightScore) || getCommentPublishedTime(b) - getCommentPublishedTime(a));
  const cleanForTab = (items: any[] = []) => items.slice(0, 10).map(({ classification, ...item }) => item);
  const makeTab = (key: any, label: any, items: any, latest = false) => ({ key, label, count: items.length, comments: cleanForTab(latest ? sortByLatest(items) : sortBySignal(items)) });
  const bySentiment = (sentiment: any) => enriched.filter((item) => item.sentiment === sentiment);
  const byClassification = (predicate: any) => enriched.filter((item) => predicate(item.classification || {}));
  ...
  return {
    ...
    purchaseIntent: makeTab('purchaseIntent', 'Purchase Intent', byClassification((item: any) => item.purchaseIntent)),
    priceAvailability: makeTab('priceAvailability', 'Price / Availability', byClassification((item: any) => item.priceAvailabilityQuestion)),
    questions: makeTab('questions', 'Questions', byClassification((item: any) => item.question)),
    trustSignals: makeTab('trustSignals', 'Trust Signals', byClassification((item: any) => item.trustSignal)),
    productProof: makeTab('productProof', 'Proof Requests', byClassification((item: any) => item.productProofQuestion)),
    sponsorshipConcerns: makeTab('sponsorshipConcerns', 'Sponsorship', byClassification((item: any) => item.sponsorshipConcern)),
    spamIrrelevant: makeTab('spamIrrelevant', 'Spam / Low Value', byClassification((item: any) => item.spamIrrelevant))
  };
}

function buildCommonThemes(counts: any) {
  const themes: any[] = [];
...
function getSentimentLabel(sentiment: any = {}) {
...
function analyzeComments(comments: any[] = [], replies: any[] = [], options: any = {}) {
  ...
  (sentiment as any).label = getSentimentLabel(sentiment);
...
function inferContentFormat(videoMetrics: any) {
...
function analyzeContentQuality(videoMetrics: any) {
...
function inferInfluencerCategory(videoMetrics: any, channelMetrics: any, contentQuality: any) {
...
function metric(value: any, displayValue: any, formula: any, note = '') {
...
function buildPerformanceEstimates({ videoMetrics, commentIntelligence, creatorAverage, contentQuality, input = {} }: any) {
...
function calculateScores(videoMetrics: any, comments: any, channelMetrics: any, creatorAverage: any, estimates: any) {
  const engagementRate = toNumber(videoMetrics.engagementRate);
  const views = toNumber(videoMetrics.viewCount);
...
function compareMetric(current: any, base: any, decimals = 2) {
...
function buildChannelBaseline(videoMetrics: any, creatorAverage: any) {
...
function determineVerdict(scores: any, creatorInsights: any) {
...
function getVideoType(videoMetrics: any) {
...
function buildChannelInsightsFromBaseline(channelMetrics: any, creatorInsights: any, channelBaseline: any) {
...
function buildEstimatedWatchTimeDocument(performanceEstimates: any) {
  return {
...
function buildEstimatedRevenueDocument(videoMetrics: any, performanceEstimates: any) {
...
function buildPerformanceComparisonDocument(videoMetrics: any, creatorAverage: any, channelBaseline: any) {
  return {
    currentVideo: {
...
function buildAdvancedCommentInsights(commentIntelligence: any) {
...
function shouldPersistReport(payload: any = {}) {
...
export async function createInsightReport({ actor = {}, payload = {} }: any) {
  const input = buildYoutubeOnlyInput(payload);
  const videoId = extractYouTubeVideoId(input.videoUrl);
  if (!videoId) {
    const error: any = new Error('Invalid YouTube video URL or video ID. Send videoUrl, youtubeUrl, videoLink, link, or a raw 11-character videoId.');
  ...
  // Stored as one JSON document with the list-view columns lifted out alongside it — see the
  // InsightReport model for why this isn't shredded into relational tables.
  const saved = await prisma.insightReport.create({
    data: {
      ...
      channelTitle: channelMetrics.title,
      thumbnailUrl: videoMetrics.thumbnailUrl || null,
      viewCount: Math.round(toNumber(videoMetrics.viewCount)),
      engagementRate: toNumber(videoMetrics.engagementRate),
      finalScore: toNumber(scores?.finalAiScore),
      verdict: clean(finalVerdict?.verdict) || null,
      publishedAt: videoMetrics.publishedAt,
      data: reportPayload as any,
    },
  });
