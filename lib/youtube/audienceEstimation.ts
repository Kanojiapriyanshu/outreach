/**
 * Category-benchmark audience estimation.
 *
 * Ported from the Fidem CRM's `server/services/audienceEstimation.service.js`, kept
 * byte-for-byte in behaviour (benchmark tables, keyword hints, rounding, format-skew formula)
 * rather than rewritten, for the same reason insightReport.ts keeps its own port exact — reports
 * from this and the old system should score the same content the same way.
 *
...
const CATEGORY_KEYWORD_HINTS: [string, RegExp][] = [
  ...
  ["beauty", /\b(beauty|makeup|skincare|cosmetic|haircare)\b/i],
