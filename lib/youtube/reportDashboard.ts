/**
 * Shapes a stored insight report into the structure the Insight OS UI renders — headline cards,
 * chart series, and the summary row used in the reports list.
 *
 * Ported from `server/services/youtubeReportDashboard.service.js`. Presentation rules only; the
 * numbers themselves are produced upstream in insightReport.ts.
 */
import {
  toNumber,
  round,
...
function safeDivide(n: any, d: any) {
...
function buildChannelUrl(channel: any = {}) {
...
function buildVideoUrl(report: any = {}) {
...
function buildProfile(report: any) {
  const channel = report.channelMetrics || {};
...
function buildVideoOverview(report: any) {
  const video = report.videoMetrics || {};
  const hero = buildHero(report);
  const creator = report.creatorInsights || report.influencerInsights || {};
  return {
...
function buildOverviewCards(report: any) {
  const video = report.videoMetrics || {};
...
function buildChannelOverview(report: any) {
  const channel = report.channelMetrics || {};
  return {
    name: channel.title || '',
...
function buildPerformanceEstimateCards(report: any) {
...
function buildEstimatedWatchTime(report: any) {
  const e = report.performanceEstimates || {};
  return {
...
export function buildEstimatedRevenue(report: any) {
  const video = report.videoMetrics || {};
  const e = report.performanceEstimates || {};
...
function buildAudienceMatch(report: any) {
...
function buildAudienceSignals(report: any) {
...
function buildTopInterests(report: any) {
  const video = report.videoMetrics || {};
  const channel = report.channelMetrics || {};
  const creator = report.creatorInsights || report.influencerInsights || {};
  return [creator.primaryCategory, creator.youtubeCategory, ...(creator.secondaryCategories || []), ...(video.tags || []), ...(channel.topicDetails?.topicCategories || []), creator.contentFormat]
    .filter(Boolean)
    .map((item: any) => (String(item).split('/').pop() ?? '').replace(/_/g, ' '))
    .filter(Boolean)
    .slice(0, 60);
}

function buildCreatorFit(report: any) {
  const creator = report.creatorInsights || report.influencerInsights || {};
  return {
    title: 'Creator Fit',
...
function buildCommentRows(report: any) {
...
function buildCommentBreakdown(report: any) {
  const comment = report.commentIntelligence || {};
  const sentiment = comment.sentiment || {};
  const themeRows = [
    ...(comment.commonThemes || []),
    ...(comment.topQuestions || []).map((theme: any) => ({ theme, type: 'question' })),
    ...(comment.topPurchaseIntentExamples || []).map((theme: any) => ({ theme, type: 'purchase_intent' })),
    ...(comment.topTrustExamples || []).map((theme: any) => ({ theme, type: 'trust' })),
    ...(comment.topObjections || []).map((theme: any) => ({ theme, type: 'objection' }))
  ].slice(0, 20);
  return {
...
function normalizeRecentVideo(video: any) {
...
function buildLastVideosComparison(report: any) {
  const baseline = report.channelBaseline || report.performanceComparison || {};
  const current = report.videoMetrics || {};
  const rows = (report.rawData?.recentVideosSample || []).map(normalizeRecentVideo).filter((item: any) => item.videoId).slice(0, 10);
  const buildChange = (value: any, avg: any) => avg ? round(((toNumber(value) - toNumber(avg)) / toNumber(avg)) * 100, 1) : null;
  return {
...
function buildPerformanceComparison(report: any) {
  const baseline = report.channelBaseline || report.performanceComparison || {};
  return {
...
function buildContentPerformanceSummary(report: any) {
  const video = report.videoMetrics || {};
...
function buildAiSummary(report: any) {
...
function buildScoreCards(report: any) {
...
function buildOtherDeliverables(report: any) {
  const hero = buildHero(report);
  const profile = buildProfile(report);
  return (report.rawData?.recentVideosSample || []).map(normalizeRecentVideo).filter((item: any) => item.videoId && item.videoId !== report.videoId).slice(0, 3).map((item: any) => ({
...
export function buildChartData(report: any) {
  const comment = report.commentIntelligence || {};
  const sentiment = comment.sentiment || {};
  return {
    sentimentDonut: [
...
export function buildYoutubeInsightDashboard(inputReport: any) {
...
export function formatYoutubeInsightReport(report: any, options: any = {}) {
