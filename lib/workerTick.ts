 ...
 * follow-ups, record a heartbeat. Shared by the standalone always-on script (scripts/worker.ts,
 * for anyone self-hosting a persistent process) and the cron-triggered API route
 * (app/api/cron/tick) used when there's no persistent worker host — a scheduled GitHub Actions
 * job hits that route on an interval instead.
 */
