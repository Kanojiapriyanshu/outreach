 ...
 *   video → score the creator → tier → rank → return evidence
 *
...
export type CampaignProgressEvent =
  | { type: "stage"; stage: "search" | "channels" | "analysis" | "ranking" | "done"; message: string }
...
export async function runCampaignDiscovery(
  ...
  while (queue.length > 0 && queriesRun.length < settings.maxQueries) {
    ...
    if (responses.every((r) => r === null)) {
      if (pool.size === 0) {
        await recordUnitsUsed(unitsUsed).catch(() => undefined);
  ...
  await saveToLibrary(creators.filter((c) => c.tier !== "D"), profile);
  await recordUnitsUsed(unitsUsed).catch((err) => console.error("[Campaign discovery] Couldn't record API units:", err));
...
/** Qualifying creators land in the Library like any other discovery result, so outreach and media
 * kits work on them straight away. A save failure is logged, never allowed to lose the run. */
async function saveToLibrary(creators: QualifiedCreator[], profile: CampaignProfile): Promise<void> {
  if (creators.length === 0) return;
  try {
    const existing = await prisma.creator.findMany({ where: { channelId: { in: creators.map((c) => c.channelId) } }, select: { channelId: true } });
    const existingIds = new Set(existing.map((e) => e.channelId));
    const niche = profile.targetProducts.join(", ") || profile.category;

    await Promise.all(
      creators.map(async (c) => {
        c.alreadyInLibrary = existingIds.has(c.channelId);
        try {
          const platformTags = Object.keys(c.platformLinks);
          const row = await prisma.creator.upsert({
            ...
            create: {
              ...
              lastUploadAt: c.lastUploadAt ? new Date(c.lastUploadAt) : null,
              platformLinks: c.platformLinks as object,
              platformTags,
              discoverySource: "campaign",
              lastDiscoveredAt: new Date(),
            },
            // Same rule as Discovery's quick search: refresh the numbers, never overwrite a name,
            // niche or email the team may already have edited by hand.
            update: {
              thumbnailUrl: c.thumbnailUrl,
              description: c.description,
              country: c.country || null,
              subscriberCount: c.subscriberCount,
              ...(c.averageViews !== null ? { averageViews: c.averageViews } : {}),
              ...(c.engagementRate !== null ? { engagementRate: c.engagementRate } : {}),
              lastUploadAt: c.lastUploadAt ? new Date(c.lastUploadAt) : null,
              platformLinks: c.platformLinks as object,
              platformTags,
              lastDiscoveredAt: new Date(),
          ...
          console.error(`[Campaign discovery] Couldn't save ${c.channelId} to the Library:`, err);
        }
      })
    );
  } catch (err) {
    console.error("[Campaign discovery] Library save failed:", err);
  }
}
