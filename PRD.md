...
- **Roster** (`/influencers/creators`, filters in `lib/creatorRoster.ts`): one row per Creator — audience, email + where it came from (`emailSource`), platform links, websites, latest rate (copied onto the Creator by `lib/creatorProfileSync.ts` whenever a thread records one), outreach stage, media kit link. Actions: find emails, make media kits, edit (reuses Discovery's detail modal), pitch, CSV export (`/api/influencers/creators/export`).
- **Auto-filled Email 1** (`lib/outreachPersonalization.ts`, pure): `{Content_Highlights}` from topics that recur in the creator's recent video titles; `{Niche_Or_Product_Category}` / `{Deliverable_Type}` from the campaign, else the channel's strongest subject / "dedicated review". `personalizeCreator()` in `lib/creatorEnrichment.ts` reads uploads (3 YouTube units; highlights cached 30 days on the Creator) and renders the live template. Used by Discovery's Start Outreach (Compose opens pre-written), New Outreach's "Fill in from their channel", and the bulk pitch dialog.
- **Bulk pitch** (`POST /api/influencers/bulk-send`): every email is queued as a `ScheduledInitialEmail` (sending window by default); the worker sends them one at a time with the usual spacing, quota and suppression checks, then the normal influencer cadence runs.
- **Email lookup** (`lib/creatorEmailFinder.ts`): channel description (including "[at]/[dot]" obfuscation), link-in-bio pages, and websites the creator evidently owns plus their contact pages. Never fetches YouTube's hidden-email button, Instagram/TikTok/Facebook, stores or sponsor/affiliate links; honours robots.txt; blocks private/internal addresses; at most 8 pages / ~20s per creator. An email is only saved with an ownership signal (score ≥ 40). Runs automatically for creators never checked (`emailCheckedAt` null) and on demand.
- **Media kits:** any creator reply except an opt-out queues one (`mediaKitRequestedAt`); `ensureCreatorMediaKit()` reuses a kit younger than 30 days, stores `mediaKitShareToken`, and logs `MEDIA_KIT_READY` on the thread.
- **Worker:** `runWorkerTick()` lazily loads `runCreatorEnrichmentPass()` after the sends and inbox passes, only while the tick is under 25s old, with a ~20s budget: one queued media kit, then email lookups.
