  ...
  return (
    <div className="space-y-7">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-[22px] font-semibold tracking-tight text-[var(--ink)]">Pipeline</h1>
          <p className="text-sm text-[var(--muted)] mt-0.5">
            Every outreach the automation is tracking — search, filter and drill into any of them.
          </p>
        </div>
        <Link
          href="/track"
...
function TabLink({
  ...
  return (
    <Link
      href={`/pipeline?${buildQuery(extraParams, { tab })}`}
  ...
  return (
    <div className="flex items-center justify-between gap-3 flex-wrap text-sm">
      ...
      <div className="flex gap-2">
        <Link
          href={`/pipeline?${buildQuery(extraParams, { page: String(page - 1) })}`}
